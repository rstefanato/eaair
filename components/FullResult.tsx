  ...
  return (
    <section className="bg-off-white px-5 py-12">
      <div className="mx-auto max-w-[600px]">
        ...
        <StaggerChildren className="space-y-3" staggerDelay={0.12} baseDelay={0.3}>
          {benefits.map((b) => (
            <motion.div
              key={b.title}
              variants={staggerItem}
              className="flex gap-4 rounded-2xl border-[1.5px] border-border bg-white p-4 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5"
            >
        ...
        <FadeIn delay={0.5} className="mt-8 text-center">
          <p className="font-body text-sm text-text-mid">O próximo passo é consultar um especialista credenciado perto de você.</p>
        </FadeIn>
      </div>
    </section>
  );
